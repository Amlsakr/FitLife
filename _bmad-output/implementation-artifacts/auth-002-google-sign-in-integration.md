...
- [ ] Add modern Google sign-in dependencies and configuration. (AC: 1, 2, 7)
  - [ ] Add the required Credential Manager and Google ID artifacts to `gradle/libs.versions.toml` using the project's version-catalog pattern.
  - [ ] Update `feature/auth/auth-data/build.gradle.kts` and `feature/auth/auth-ui/build.gradle.kts` with only the libraries this story needs.
  - [ ] Keep the existing Firebase BoM and `google-services` plugin flow; do not add a second Firebase setup path.
  ...
  - [ ] Do not introduce legacy Google Sign-In or One Tap APIs.
- [ ] Extend the auth-domain contract for Google sign-in. (AC: 2-7)
  - [ ] Add a Google sign-in repository method that accepts a Google ID token as a plain string and stays free of Android/Firebase SDK types.
  - [ ] Add a focused single-`operator fun invoke` use case for Google sign-in.
  - [ ] Add an explicit domain error for post-auth account/profile initialization failure if the Firestore user document cannot be written after Firebase Auth succeeds.
  - [ ] Keep cancellation as a silent no-op at the UI boundary; do not convert a user-dismissed Google picker into a visible auth failure.
- [ ] Implement the Firebase-backed Google auth and Firestore user initialization path. (AC: 2, 5, 6, 7)
  - [ ] Exchange the Google ID token for a Firebase credential with `signInWithCredential`.
  - [ ] Upsert the Firestore `users/{uid}` document idempotently and preserve existing profile/onboarding fields with merge semantics.
  - [ ] Map provider, credential, network, and backend failures to explicit domain errors through one dedicated mapper.
  - [ ] Re-throw `CancellationException` and keep Google SDK/Firebase types inside `auth-data`.
  - [ ] Add Hilt bindings/providers in the existing auth-data module style.
- [ ] Add the Google sign-in UI flow to the existing auth screen. (AC: 1, 3, 4, 8)
  - [ ] Extend the current auth MVI state/event/action flow instead of creating a separate Google-only screen.
  - [ ] Keep Credential Manager and Google credential parsing in `auth-ui`; the ViewModel should only receive the resulting ID token and drive loading/navigation state.
  - [ ] Add a distinct Google button to the auth UI and preserve the current email/password validation, sign-up, verification, resend, refresh, and sign-out flows.
  - [ ] Use the same authenticated-navigation callback path already used by AUTH-001 after the Google token exchange succeeds.
  - [ ] Ensure a dismissed Google picker leaves the current screen unchanged and does not emit an error snackbar.
- [ ] Wire the new Google path into the app composition root. (AC: 2, 3, 8)
  - [ ] Update `MainActivity` only as needed to supply the new Google use case into the auth ViewModel construction.
  - [ ] Preserve the existing Navigation 3 startup and auth root-replacement behavior.
  - [ ] Do not create a second auth navigation state machine or bypass the existing verified-session checks.
- [ ] Add focused verification. (AC: 1-8)
  - [ ] Unit-test Google token mapping, cancellation handling, error mapping, and Firestore user upsert semantics.
  - [ ] Unit-test the auth UI events/actions for Google button launch, success, dismissal, and retryable failure behavior.
  - [ ] Update the app-level navigation tests to confirm the Google auth path still replaces Auth with the correct typed destination.
  - [ ] Keep automated tests emulator-first and offline-safe; do not rely on live Google accounts or production Firebase data.
